...
- 在静态构造函数中，通过 C# 9.0+ 的 `switch` 模式匹配一次性初始化完毕。
...
### E. 智能目标提取与零正则开销
废弃了原本笨重的正则表达式（不再尝试匹配 `TargetA` 等无用字符）。直接依赖现代 C# 语法进行提取：
```csharp
...
```
结合内部的 `HashSet<string> _targetNames` 实现自动去重，提取逻辑极其干净高效。
...
========================================================================================
 ...
 [流式追加分支 (Append Pipeline)]                         [全新建档分支 (New Session Pipeline)]
   ...
   |-- 3. GenerateAggregatedContent() (生成如"几小时内搬运了N次") |-- 3. 实例化全新的 MemoryEntry 并移交维护层 ActiveMemories
   |-- 4. ImportanceIncrement() (重要性增量计算)                  `-- 4. 强引用锚定 _lastJobMemory = newMemory
   |                                                              
   |-- 5. 基于手里握着的 _lastJobMemory 引用进行直接内存覆写：    v
   |        - _lastJobMemory.Content = newText                 (结束)
   |        - _lastJobMemory.Importance += increment
   |        - _lastJobMemory.GameTick = Now
   |
   v
