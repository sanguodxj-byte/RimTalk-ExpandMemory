...
namespace RimTalk.Memory.Patches
{
    ...
    {
        private static bool IsEnabled => RimTalkMemoryPatchMod.Settings?.IsRoundMemoryActive ?? false; // 通过设置控制启用
        ...
        static void Postfix(List<TalkResponse> responses)
        {
            ...
            // 所以这里还是决定提前检查一下
            if (!IsEnabled || Current.Game == null) return;
            ...
            var pawns = responses
                ...
                .Where(p => p != null)
                .ToHashSet();
            bool isPlayerInitiate = responses.FirstOrDefault(r => r != null)?.TalkType == TalkType.User;

            // 将数据传给RoundMemoryManager
            RoundMemoryManager.BuildRoundMemory(content, pawns, isPlayerInitiate);
        }
