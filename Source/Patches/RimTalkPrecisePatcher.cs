    ...
    public static class RimTalkPrecisePatcher
    {
        ...
        private static void BuildContext_Postfix(ref string __result, List<Pawn> pawns)
        {
            try
            {
                ...
                var memoryComp = mainPawn?.TryGetComp<PawnMemoryComp>();
                ...
                if (!string.IsNullOrEmpty(memoryContext))
                {
                    ...
                    Log.Message($"[RimTalk Patcher {VERSION}] ✓ Injected memory for {mainPawn.LabelShort} into BuildContext"); // <-- 修改日志
                }
            }
            catch (Exception ex)
            {
        ...
        private static void DecoratePrompt_Postfix(object talkRequest, List<Pawn> pawns)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(memoryContext))
                {
                    ...
                    Log.Message($"[RimTalk Patcher {VERSION}] ✓ Injected memory for {mainPawn.LabelShort} into DecoratePrompt"); // <-- 修改日志
                }
            }
            catch (Exception ex)
            {
