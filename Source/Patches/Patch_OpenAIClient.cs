...
{
    [HarmonyPatch(typeof(OpenAIClient), "GetChatCompletionAsync")]
    public static class Patch_OpenAIClient_GetChatCompletionAsync
    {
        private static readonly System.Threading.ThreadLocal<bool> _isInsidePatch = new System.Threading.ThreadLocal<bool>(() => false);

        static bool Prefix(OpenAIClient __instance, string instruction, List<(Role role, string message)> messages, ref Task<Payload> __result)
        {
        ...
        private static Task<Payload> CallOriginalMethod(OpenAIClient instance, string instruction, List<(Role role, string message)> messages)
        {
            try
            {
                _isInsidePatch.Value = true;
                MethodInfo originalMethod = typeof(OpenAIClient).GetMethod("GetChatCompletionAsync", BindingFlags.Public | BindingFlags.Instance);
            ...
            {
                Log.Error($"[RimTalk-ExpandMemory] Patch_OpenAIClient: Error calling original method: {ex}");
