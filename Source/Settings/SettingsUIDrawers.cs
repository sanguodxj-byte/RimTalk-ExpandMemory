...
namespace RimTalk.MemoryPatch
{
    ...
    public static class SettingsUIDrawers
    {
        ...
        public static void DrawAdvancedMatchingSettings(Listing_Standard listing, RimTalkMemoryPatchSettings settings)
        {
            ...
            listing.CheckboxLabeled("启用常识链", ref settings.enableKnowledgeChaining);
            ...
            // 防误触领跑分
            listing.Label($"防误触领跑分 (Confidence Margin): {settings.confidenceMargin:P0}");
            ...
            // 混合检索权重
            listing.Label($"混合检索权重 (Hybrid Weight Balance): {settings.hybridWeightBalance:P0}");
